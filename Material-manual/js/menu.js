...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // ============================================
  // 4. МЕНЮ - СУБ.ПАРТНЕРСТВО
  // ============================================
  const menuItems = document.querySelectorAll('.menu-item');
  menuItems.forEach(item => {
  const span = item.querySelector('span');
  if (span && span.textContent.trim() === 'СУБ.ПАРТНЕРСТВО') {
    item.addEventListener('click', function(e) {
      e.preventDefault();
      openModal('.sub_partner');
    });
  }
  
  // РЕФ. ПРОГРАММА
  if (span && span.textContent.trim() === 'РЕФ. ПРОГРАММА') {
    item.addEventListener('click', function(e) {
      e.preventDefault();
      openModal('.sub_partner.ref_program');
    });
  }
  });
  ...
  // Глобальные функции для показа/скрытия полей креативов
  window.showCreativeFields = function() {
    const mediaTypeSelector = document.querySelector('.media-type-selector.creative-only');
    if (mediaTypeSelector) {
      mediaTypeSelector.style.display = 'flex';
    }
    
    const activeType = document.querySelector('.media-type-btn.active')?.dataset.type || 'image';
    
    if (activeType === 'image') {
      document.querySelectorAll('.image-field.creative-only').forEach(el => el.style.display = 'flex');
      document.querySelectorAll('.video-field.creative-only').forEach(el => el.style.display = 'none');
    } else {
      document.querySelectorAll('.image-field.creative-only').forEach(el => el.style.display = 'none');
      document.querySelectorAll('.video-field.creative-only').forEach(el => el.style.display = 'flex');
    }
    
    document.querySelectorAll('.log_type.creative.creative-only:not(.media-field):not(.file-upload)').forEach(el => {
      el.style.display = 'flex';
    });
    
    // Скрываем поля мануалов
    document.querySelectorAll('.manual-only').forEach(el => el.style.display = 'none');
  };
  
  window.hideCreativeFields = function() {
    document.querySelectorAll('.creative-only').forEach(el => {
      el.style.display = 'none';
    });
  };
  
  // НЕ вызываем setupContentAlert при загрузке - пусть будут видны обычные поля
  
  // ============================================
  // ИНИЦИАЛИЗАЦИЯ
  // ============================================
  setupModalClose();
  setupMobileWithdrawButton();

  // ============================================
  // КНОПКА ВЫВОДА В МОБИЛЬНОМ МЕНЮ
  // ============================================
  function setupMobileWithdrawButton() {
    // Находим кнопку "Вывод" в мобильном меню
    const withdrawLinks = document.querySelectorAll('.menu-mobile .funds a');
    let withdrawLink = null;
    
    withdrawLinks.forEach(link => {
      if (link.textContent.trim().includes('Вывод')) {
        withdrawLink = link;
      }
    });
    ...
    if (withdrawLink) {
      withdrawLink.addEventListener('click', function(e) {
        ...
        const now = new Date();
        const dayOfWeek = now.getDay(); // 0 - воскресенье, 1 - понедельник, 2 - вторник и т.д.
        ...
        if (dayOfWeek === 2 && currentTime >= startTime && currentTime < endTime) {
          ...
          // Показываем окно с недоступным выводом
          openModal('.witd_funds');
        }
      });
    }
